...
type ListingCreationStore = {
	...
	setMortgageField: <K extends keyof Omit<
		MortgageFormState,
		'address' | 'location'
	>>(field: K, value: MortgageFormState[K]) => void;
...
export const useListingCreationStore = create<ListingCreationStore>((set, get) => ({
	...
	applyBorrowerSuggestion: (suggestion) =>
		set((state) => {
			const nextErrors = ['borrower.name', 'borrower.email', 'borrower.rotessaCustomerId'].reduce(
				(acc, key) => removeError(acc, key),
				state.errors
			);
			return {
				borrower: {
	...
	removeImage: (index) =>
		set((state) => {
			const nextImages = state.images.filter((_, i) => i !== index).map((image, i) => ({
				...image,
				order: i,
			}));
	...
	clearError: (field) => set((state) => ({ errors: removeError(state.errors, field) })),
...
export const validateListingForm = ({
	...
	images,
	documents,
}: Pick<ListingCreationStore, 'borrower' | 'mortgage' | 'listing' | 'images' | 'documents'>) => {
	...
	if (mortgage.originationDate && mortgage.maturityDate) {
		...
		if (!Number.isNaN(origination) && !Number.isNaN(maturity) && origination > maturity) {
			errors['mortgage.maturityDate'] = 'Maturity date must be after origination date';
		}
	}
	...
	if (!Number.isFinite(appraisalMarketValue) || appraisalMarketValue <= 0) {
		errors['mortgage.appraisalMarketValue'] = 'Appraisal value must be positive';
	}
	...
	if (!mortgage.appraisalDate) {
		...
		if (Number.isNaN(parsedDate)) {
			...
			errors['mortgage.appraisalDate'] = 'Appraisal date cannot be in the future';
		}
	}
	...
	if (!mortgage.externalMortgageId.trim()) {
		errors['mortgage.externalMortgageId'] = 'External mortgage ID is required for webhook idempotency';
	}
