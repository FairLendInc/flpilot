...
const TYPE_KEYWORDS: Record<string, { keywords: string[], patterns: RegExp[], weight: number }> = {
  'appraisal': {
    keywords: ['appraisal', 'property value', 'market value', 'comparable', 'valuation', 'assessment', 'fair market value'],
    patterns: [/appraisal/i, /property value/i, /market value/i, /comparable/i],
    weight: 1.0
  },
  'title': {
    keywords: ['title', 'deed', 'ownership', 'property title', 'legal description', 'chain of title'],
    patterns: [/title report/i, /deed/i, /ownership/i, /legal description/i],
    weight: 1.0
  },
  'inspection': {
    keywords: ['inspection', 'property inspection', 'home inspection', 'defects', 'repairs', 'structural'],
    patterns: [/inspection report/i, /home inspection/i, /property condition/i],
    weight: 1.0
  },
  'loan_agreement': {
    keywords: ['loan agreement', 'mortgage', 'promissory note', 'interest rate', 'principal', 'amortization', 'borrower', 'lender'],
    patterns: [/loan agreement/i, /mortgage agreement/i, /promissory note/i, /note and deed of trust/i],
    weight: 1.0
  },
  'insurance': {
    keywords: ['insurance', 'policy', 'coverage', 'premium', 'deductible', 'claim', 'property insurance', 'hazard insurance'],
    patterns: [/insurance policy/i, /policy number/i, /certificate of insurance/i, /declarations page/i],
    weight: 1.0
  },
  'tax_document': {
    keywords: ['tax', 'property tax', 'tax bill', 'assessment', 'tax statement', 'irs'],
    patterns: [/tax bill/i, /property tax/i, /tax statement/i, /form \d+/i],
    weight: 0.9
  },
  'bank_statement': {
    keywords: ['bank statement', 'account statement', 'balance', 'transaction', 'deposit', 'withdrawal'],
    patterns: [/bank statement/i, /account statement/i, /statement of account/i],
    weight: 0.9
  },
  'pay_stub': {
    keywords: ['pay stub', 'paycheck', 'earnings', 'gross pay', 'net pay', 'year to date'],
    patterns: [/pay stub/i, /earnings statement/i, /pay advice/i],
    weight: 0.9
  },
  'identity_document': {
    keywords: ['driver license', 'passport', 'state id', 'identification', 'birth certificate'],
    patterns: [/driver license/i, /passport/i, /state id/i, /identification/i],
    weight: 0.8
  },
  'closing_disclosure': {
    keywords: ['closing disclosure', 'cd', 'loan estimate', 'settlement statement', 'hud', 'closing costs'],
    patterns: [/closing disclosure/i, /loan estimate/i, /settlement statement/i, /hud-1/i],
    weight: 1.0
  }
};
...
export const analyzeFilename = internalQuery({
  ...
  handler: async (ctx, args): Promise<FilenameAnalysis> => {
    const filename = args.filename.toLowerCase();
    const extension = args.extension || filename.split('.').pop() || '';
    ...
    const extensionClues: Record<string, string> = {
      'pdf': 'document',
      'doc': 'document',
      'docx': 'document',
      'xls': 'spreadsheet',
...
export const analyzeDocumentContent = internalQuery({
  ...
  handler: async (ctx, args): Promise<DocumentContentAnalysis> => {
    ...
    const confidenceScore = calculateContentConfidence(content, filename, contentFeatures);
...
export const generateTypeSuggestions = internalQuery({
  args: {
    contentAnalysis: v.object({
      ...
      contentFeatures: v.object({
        ...
        documentStructure: v.union(v.literal('simple'), v.literal('structured'), v.literal('complex')),
      }),
  ...
  handler: async (ctx, args): Promise<TypeSuggestion[]> => {
    ...
    const allDocumentTypes = await ctx.db
    ...
    // Get document groups
    const allDocumentGroups = await ctx.db
    ...
    const groupMap = new Map(documentGroups.map(group => [group.name, group]));

    // Score each document type
    for (const docType of documentTypes) {
      const score = calculateTypeScore(docType.name, contentAnalysis, filenameAnalysis);

      if (score.confidenceScore > 0.1) { // Only include types with meaningful scores
...
export const getUserAutoAssignmentSettings = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      autoAssignmentEnabled: user?.preferences?.autoDocumentTypeAssignment || false,
      confidenceThreshold: user?.preferences?.autoAssignmentThreshold || 0.8,
      requireConfirmation: user?.preferences?.requireConfirmationForAutoAssignment || true
    };
  },
});
...
function analyzeContentFeatures(content: string): ContentFeatures {
  return {
    ...
    hasSignatures: /signature|signed|sign here/i.test(content),
    hasDates: /\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}|\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}/.test(content),
    hasMonetaryValues: /\$\s*\d{1,3}(?:,\d{3})*(?:\.\d{2})?/.test(content),
    hasLegalLanguage: /hereby|whereas|notwithstanding|pursuant to|in witness whereof/i.test(content),
    hasPropertyAddresses: /\d+\s+[\w\s]+\s+(street|st|avenue|ave|road|rd|drive|dr|lane|ln|boulevard|blvd)/i.test(content),
    hasLoanNumbers: /loan\s+#|loan\s+number|account\s+#|mortgage\s+#/i.test(content),
    hasInsurancePolicyNumbers: /policy\s+#|policy\s+number|insurance\s+#/i.test(content),
    documentStructure: analyzeDocumentStructure(content)
  };
}

function analyzeDocumentStructure(content: string): 'simple' | 'structured' | 'complex' {
  ...
  if (hasHeaders && hasMultipleSections && hasLists) {
    return 'complex';
  } else if (hasHeaders || hasMultipleSections) {
    return 'structured';
  } else {
    return 'simple';
  }
...
function calculateKeywordDensity(content: string): Record<string, number> {
  ...
  const importantWords = ['appraisal', 'title', 'inspection', 'loan', 'mortgage', 'insurance', 'tax', 'property', 'policy', 'agreement'];
...
function calculateContentConfidence(content: string, filename: string, features: ContentFeatures): number {
  let confidence = 0;
...
function calculateTypeScore(
  ...
  const filenameScore = filenameAnalysis.suggestedTypes.includes(normalizedTypeName) ? 0.2 : 0;
  if (filenameScore > 0) {
    score += filenameScore;
  ...
  if (['loan_agreement', 'mortgage'].includes(normalizedTypeName) && contentFeatures.hasMonetaryValues) {
    score += 0.1;
    reasoning.push('Contains monetary values typical of loan documents');
    matchedFeatures.push('monetary_values');
  }

  if (['title', 'deed'].includes(normalizedTypeName) && contentFeatures.hasLegalLanguage) {
    score += 0.1;
    reasoning.push('Contains legal language typical of title documents');
    matchedFeatures.push('legal_language');
  }

  if (['appraisal', 'inspection'].includes(normalizedTypeName) && contentFeatures.hasPropertyAddresses) {
    score += 0.1;
...
export const autoAssignDocumentType = authMutation({
  ...
  handler: async (ctx, args): Promise<{
    ...
    const settings = await ctx.runQuery(internal.documentAnalysis.getUserAutoAssignmentSettings, {
    ...
    const contentAnalysis: DocumentContentAnalysis = await ctx.runQuery(internal.documentAnalysis.analyzeDocumentContent, {
    ...
    // Analyze filename
    const filenameAnalysis = await ctx.runQuery(internal.documentAnalysis.analyzeFilename, {
      filename: args.filename,
    });

    // Generate type suggestions
    const suggestions: TypeSuggestion[] = await ctx.runQuery(internal.documentAnalysis.generateTypeSuggestions, {
      contentAnalysis,
      filenameAnalysis,
    ...
    const confidenceThreshold: number = args.forceAssignment ? 0.5 : settings.confidenceThreshold;
    ...
    const result = {
      ...
      requireConfirmation: settings.requireConfirmation && !args.forceAssignment,
      allSuggestions: suggestions
    };
...
export const getDocumentTypeSuggestions = authMutation({
  ...
  handler: async (ctx, args): Promise<{
    ...
    return {
      ...
      contentAnalysis,
      filenameAnalysis,
      hasConfidentSuggestion: suggestions.length > 0 && suggestions[0].confidenceScore > 0.7
    };
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    if (documentType.validationRules) {
      ...
      if (rules.maxSize && args.fileSize > rules.maxSize) {
        errors.push(`File size ${(args.fileSize / 1024 / 1024).toFixed(1)}MB exceeds maximum allowed size of ${(rules.maxSize / 1024 / 1024).toFixed(1)}MB`);
      }

      // File format validation
      if (rules.allowedFormats && !rules.allowedFormats.includes(args.fileFormat.toLowerCase())) {
        errors.push(`File format "${args.fileFormat}" is not allowed. Allowed formats: ${rules.allowedFormats.join(', ')}`);
      }
      ...
      if (args.content && rules.requiredFields) {
        ...
        for (const field of rules.requiredFields) {
          if (!contentLower.includes(field.toLowerCase())) {
            warnings.push(`Document may be missing required information: ${field}`);
          }
        }
      }
    }
    ...
    const typeValidation = validateTypeSpecificRequirements(normalizedType, args.filename, args.content);
...
function validateTypeSpecificRequirements(
  ...
  switch (typeName) {
    case 'appraisal':
      if (content && !content.toLowerCase().includes('value')) {
        warnings.push('Appraisal document should contain property valuation information');
      }
      break;

    case 'loan_agreement':
    case 'mortgage':
      if (content && !content.toLowerCase().match(/interest.*rate|principal|borrower|lender/)) {
        warnings.push('Loan agreement should contain key terms like interest rate, principal, borrower, or lender');
      }
      break;

    case 'insurance':
      if (content && !content.toLowerCase().match(/policy.*number|coverage|premium/)) {
        warnings.push('Insurance document should contain policy number, coverage, or premium information');
      }
      break;

    case 'title':
      if (content && !content.toLowerCase().match(/legal.*description|property.*description|ownership/)) {
        warnings.push('Title document should contain legal description or ownership information');
      }
      break;

    case 'inspection':
      if (content && !content.toLowerCase().match(/inspection|condition|defect|repair/)) {
        warnings.push('Inspection report should contain condition or defect information');
      }
      break;
  }
  ...
  if (!filenameLower.includes(typeName) && !content?.toLowerCase().includes(typeName)) {
    warnings.push(`Filename doesn't clearly indicate document type "${typeName}"`);
  }
