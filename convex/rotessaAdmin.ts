...
/**
 * Get all Rotessa schedules that are not linked to any mortgage
 *
 * Fetches all customers with their schedules, then filters out
 * schedules that already have a mortgage.rotessaScheduleId link.
 */
export const getUnlinkedSchedules = action({
	...
	handler: async (ctx) => {
		try {
			...
			// Collect all schedules from all customers
			...
			for (const customer of customers) {
				try {
					...
					const schedules = detail.transaction_schedules ?? [];
			...
			// Get all linked schedule IDs from mortgages
			...
			// Filter to only unlinked schedules
			const unlinkedSchedules = allSchedules.filter(
				(s) => !linkedSet.has(s.scheduleId)
			);

			logger.info("getUnlinkedSchedules: Found schedules", {
				total: allSchedules.length,
				linked: linkedScheduleIds.length,
				unlinked: unlinkedSchedules.length,
			});
...
/**
 * Link a Rotessa schedule to a mortgage with optional backfill
 *
 * Validates that:
 * 1. The schedule exists in Rotessa
 * 2. The mortgage doesn't already have a schedule
 * 3. The schedule isn't already linked to another mortgage
 *
 * Optionally triggers historical payment backfill.
 */
export const linkScheduleToMortgage = action({
	args: {
		mortgageId: v.id("mortgages"),
		scheduleId: v.number(),
		triggerBackfill: v.boolean(),
	},
	returns: v.object({
		success: v.boolean(),
	...
	handler: async (ctx, args) => {
		try {
			...
			// 1. Validate schedule exists in Rotessa
			let schedule: RotessaTransactionSchedule;
			try {
				schedule = await client.transactionSchedules.get(args.scheduleId);
			} catch (err) {
				logger.error("linkScheduleToMortgage: Schedule not found in Rotessa", {
					scheduleId: args.scheduleId,
					error: err,
				});
				return {
					success: false,
					error: `Schedule ${args.scheduleId} not found in Rotessa`,
				};
			}

			// 2. Get mortgage and borrower for validation
			...
			if (mortgage.rotessaScheduleId) {
				return {
					success: false,
					error: `Mortgage is already linked to schedule ${mortgage.rotessaScheduleId}`,
				};
			}

			// 3. Check if schedule is already linked to another mortgage
			const existingLink = (await ctx.runQuery(
				internal.mortgages.getMortgageByScheduleIdInternal,
				{ scheduleId: args.scheduleId }
			)) as Doc<"mortgages"> | null;
			...
			if (existingLink) {
				return {
					success: false,
					error: `Schedule ${args.scheduleId} is already linked to mortgage ${existingLink._id}`,
				};
			}

			// 4. Check for borrower mismatch (warning only)
			// Note: Schedule doesn't include customer_id directly, so we need to
			// find the customer that owns this schedule by iterating customers
			const borrower = (await ctx.runQuery(
				internal.borrowers.getBorrowerByIdInternal,
				{ borrowerId: mortgage.borrowerId }
			)) as Doc<"borrowers"> | null;

			let borrowerMismatch = false;
			let scheduleCustomerId: number | undefined;

			// Find which customer owns this schedule
			const customers = await client.customers.list();
			for (const customer of customers) {
				try {
					const detail = await client.customers.get(customer.id);
					const ownsSchedule = detail.transaction_schedules?.some(
						(s) => s.id === args.scheduleId
					);
					if (ownsSchedule) {
			...
			if (
				...
				scheduleCustomerId &&
				borrower.rotessaCustomerId !== String(scheduleCustomerId)
			) {
				borrowerMismatch = true;
				logger.warn("linkScheduleToMortgage: Borrower mismatch", {
					mortgageId: args.mortgageId,
					scheduleId: args.scheduleId,
			...
			// 5. Link the schedule to the mortgage
			await ctx.runMutation(internal.mortgages.linkRotessaScheduleInternal, {
				mortgageId: args.mortgageId,
				rotessaScheduleId: args.scheduleId,
			});

			logger.info("linkScheduleToMortgage: Schedule linked", {
				mortgageId: args.mortgageId,
				scheduleId: args.scheduleId,
			});

			// 6. Optionally trigger backfill
