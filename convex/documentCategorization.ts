...
interface DocumentGroupResolution {
  group: string;
  displayName: string;
  icon?: string;
  color?: string;
...
interface DocumentTypeWithGroupResult {
  type: any;
  group: any;
  isLegacy: boolean;
  resolutionMethod: string;
  ...
  handler: async (ctx, args): Promise<DocumentTypeWithGroupResult> => {
    ...
    if (documentType && (args.includeInactive || documentType.isActive)) {
      ...
      return {
        type: documentType,
        group: group,
        isLegacy: LEGACY_TYPE_TO_GROUP_MAPPING.hasOwnProperty(normalizedType),
        resolutionMethod: "found",
      };
    }

    // Check if it's a legacy type
    if (LEGACY_TYPE_TO_GROUP_MAPPING.hasOwnProperty(normalizedType)) {
      const legacyGroupName = LEGACY_TYPE_TO_GROUP_MAPPING[normalizedType];
      const group = await ctx.db
        .query("document_groups")
        .withIndex("by_name", (q) => q.eq("name", legacyGroupName))
  ...
  handler: async (ctx, args): Promise<BatchResolutionResult[]> => {
    const results: BatchResolutionResult[] = await Promise.all(
      args.documents.map(async (doc): Promise<BatchResolutionResult> => {
        const resolution = await ctx.runQuery(internal.documentCategorization.resolveDocumentGroupInternal, {
          document: doc,
...
export const validateDocumentCategorization = authQuery({
  ...
  handler: async (ctx, args): Promise<ValidationResult> => {
    ...
    if (args.document.group) {
      const group = await ctx.db
        .query("document_groups")
        .withIndex("by_name", (q) => q.eq("name", args.document.group!.trim().toLowerCase()))
        .first();
    ...
    if (!documentType) {
      // Check if it's a legacy type
      if (!LEGACY_TYPE_TO_GROUP_MAPPING.hasOwnProperty(normalizedType)) {
        issues.push(`Document type "${args.document.type}" does not exist and is not a legacy type`);
      }
    ...
    if (args.document.group && documentType) {
      ...
      if (documentType.groupName !== normalizedGroup) {
        issues.push(`Type "${args.document.type}" belongs to group "${documentType.groupName}" but document is assigned to group "${args.document.group}"`);
      }
    }
...
export const getDocumentGroupsForSuggestions = authQuery({
  ...
  handler: async (ctx, args): Promise<GroupSuggestion[]> => {
    ...
    let preferredGroupName = null;
    if (args.documentType) {
      const typeInfo = await ctx.runQuery(internal.documentCategorization.getDocumentTypeWithGroupInternal, {
