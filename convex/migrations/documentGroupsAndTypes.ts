// Migration script for default document groups and types
...
export const DEFAULT_DOCUMENT_GROUPS = [
  ...
  },
  {
    ...
    description: "Property-related documentation including appraisals and titles",
...
export const migrateDefaultGroups = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(`Created ${DEFAULT_DOCUMENT_GROUPS.length} default document groups`);
  },
});
...
export const migrateDefaultTypes = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(`Created ${DEFAULT_DOCUMENT_TYPES.length} default document types`);
  },
});
...
export const getGroupForLegacyType = (type: string): string => {
  return LEGACY_TYPE_TO_GROUP_MAPPING[type] || "other";
};