...
export const createDocumentType = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!["admin", "broker"].includes(ctx.role as string)) {
      throw new Error("Permission denied: Only admins and brokers can create document types");
    }
    ...
    if (!validNamePattern.test(args.name)) {
      throw new Error("Type name can only contain letters, numbers, spaces, hyphens, and underscores");
    }
    ...
    if (existingType) {
      throw new Error(`Document type "${args.name}" already exists in group "${args.groupName}"`);
    }
    ...
    if ((ctx.role as string) === "broker" && group.isDefault) {
      ...
      if (restrictedGroups.includes(groupName)) {
        throw new Error(`Permission denied: Brokers cannot create types in group "${args.groupName}"`);
      }
    }
...
export const updateDocumentType = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!["admin", "broker"].includes(ctx.role as string)) {
      throw new Error("Permission denied: Only admins and brokers can update document types");
    }
    ...
    const legacyTypes = ["appraisal", "title", "inspection", "loan_agreement", "insurance"];
    if (legacyTypes.includes(type.name) && (ctx.role as string) !== "admin") {
      throw new Error("Permission denied: Only admins can update legacy document types");
    }
    ...
    if (args.groupName !== undefined) {
      ...
      if (existingType && existingType._id !== args.typeId) {
        throw new Error(`Document type "${type.name}" already exists in group "${args.groupName}"`);
      }
...
export const deactivateDocumentType = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if ((ctx.role as string) !== "admin") {
      throw new Error("Permission denied: Only admins can deactivate document types");
    }
...
export const getDocumentTypes = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.groupName) {
      const groupName = args.groupName.trim().toLowerCase();
      typesQuery = typesQuery.filter((q) => q.eq(q.field("groupName"), groupName));
    }
    ...
    return await Promise.all(types.map(async (type) => {
      ...
      return {
        ...type,
        group,
        isUserEditable: !legacyTypes.includes(type.name) || (ctx.role as string) === "admin",
        isUserDeletable: !legacyTypes.includes(type.name) && (ctx.role as string) === "admin",
      };
    }));
  },
});
...
export const getDocumentTypesByGroup = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const result: Record<string, { group: any, types: any[] }> = {};
